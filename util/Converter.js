...
export const toVideo = (buffer, ext) =>
  ffmpeg(
    buffer,
    ["-c:v", "libx264", "-c:a", "aac", "-ab", "128k", "-ar", "44100", "-crf", "51", "-preset", "veryfast"],
    ext,
    "mp4"
  );

export const toImage = (buffer, ext) =>
  ffmpeg(buffer, ["-c:v", "png"], ext, "png");
  ...
  return filters.map(filter => {
    return Object.entries(filter).map(([name, value]) => {
      ...
      if (typeof value === "object") {
        return `${name}=${Object.entries(value).map(([k, v]) => `${k}=${v}`).join(":")}`
      }
    }).join(",")
  }).join(",")
}